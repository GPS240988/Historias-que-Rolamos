...
export async function seedCampaignMemories(campaignId: string) {
  ...
  const seedMemories: {
    ...
    // Aventura 3: O Forte de Ferro
    {
      ...
      title: 'Aventura 3 - Parte 1: O Pedido de Socorro',
      type: 'Descoberta',
      tags: ['Forte-de-Ferro', 'Guarnição', 'Fronteira', 'Orks', 'Nível 8'],
      description: 'Os heróis recebem um pedido urgente de socorro vindo do Forte de Ferro, uma guarnição militar na fronteira norte. Relatos falam sobre o avanço de uma grande tribo Ork e criaturas das sombras. O grupo precisa viajar sob tempestades de neve para alcançar o forte antes que a primeira linha de defesa ceda.'
    },
    {
  ...
  for (let i = 0; i < seedMemories.length; i++) {
    ...
    // Seed blank hero descriptions for mock heroes to keep schema consistent
    for (const charId of activeCharIds) {
      heroDescriptions[charId] = '';
    }
